...
- ✅ **The Fix (Switching Engine Error):** Resolved an issue where the packaged app would loop "Switching Engine".
  - **The Cause:** Serving from `127.0.0.1` caused many audio proxies and the YouTube IFrame API to reject requests.
  - **The Solution:** Updated `main.js` to serve from `localhost`. Expanded `streamService.ts` with more Invidious instances and added Piped fallback for Electron. Improved `App.tsx` fallback logic with retry loops for the YouTube API.
...
- **The Workaround:** The app is fully functional in the `win-unpacked` folder. Zipping this folder creates a "portable" distribution that can be sent to other users (like the developer's brother) and run via `Melody.exe`. The `dist_electron\Melody 0.0.0.exe` (portable target) also works with the `localhost` fix.
